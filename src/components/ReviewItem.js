...
const ReviewItem = ({
    ...
    if (
      ...
      (!currentItemImageStatus.url &&
        ...
        (currentItemImageStatus.error ||
          (currentItemImageStatus.retries !== undefined &&
            currentItemImageStatus.retries < 3)))
    ) {
  ...
  const getLocationLabel = (loc) => {
    ...
    const districtLabel = loc.district
      ? citiesData.find((c) => c.value === loc.city)?.districts?.find((d) => d.value === loc.district)?.[`label_${language}`] || loc.district
      : '';
  ...
  return (
    <div
      className={`relative bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl shadow-lg border border-gray-700/50 p-4 cursor-pointer transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:border-gray-600/50 max-w-[18rem] w-full mx-auto ${isHovered ? 'ring-2 ring-purple-500/20' : ''}`}
    ...
    >
      ...
      {currentItemImageStatus?.loading ? (
        <div className="mb-2 overflow-hidden rounded-xl aspect-[4/3] bg-gray-700 flex items-center justify-center">
          <div className="w-10 h-10 border-4 border-purple-500 border-t-transparent rounded-full animate-spin"></div>
          <span className="text-sm text-gray-400 ml-3">Generating image...</span>
        </div>
      ) : imageUrlToDisplay ? (
        <div className="mb-2 overflow-hidden rounded-xl aspect-[4/3]">
          <img
        ...
        <div className="mb-2 overflow-hidden rounded-xl aspect-[4/3] bg-gray-700 flex items-center justify-center text-gray-400">
      ...
      <div className="relative flex justify-between items-start">
        <div className="flex-1">
          <h3 className="text-lg font-semibold text-gray-100 mb-2 tracking-tight">{item.itemName}</h3>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {item.location?.city && item.location.city !== "Online" && (
      ...
      {item.reviews && item.reviews.length > 0 && (
        <div className="mt-2 p-2 bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-6 h-6 bg-gradient-to-br from-purple-500 to-blue-500 rounded-full flex items-center justify-center shadow-lg">
